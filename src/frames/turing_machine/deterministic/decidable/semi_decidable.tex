...
\begin{frame}{Langage semi-décidable}
    ...
    \begin{algorithm}[H]\small
      \Algo{$\Syracuse(n\in \mathbb{N}) \in \mathbb{B}$}{
        \While{\Example<1,22-23>{$n\neq 1$}}{
          \uIf{\Structure<1-5,17-21>{$n \bmod 2 = 0$}}{\Structure<1-5,18-21>{$n\leftarrow \frac{n}{2}$};}
          \lElse{\Alert<1,6-16>{$n\leftarrow \frac{3n+1}{2}$}}
        }
        \Return \True\;
      }
    \end{algorithm}
  ...
  \on[x=25mm,bottom=-2mm]{
    \begin{tikzpicture}[turingMachine,x=20mm, y=20mm]\small
      \state[example ob=<1>,alert ob=<{11}>,structure ob=<{2-4,18}>,initial above] (s) at (0,1) {\faForward};
      ...
      \path (1) edge[bend right=5mm] node[swap]   {\smTMtransR{0}{1}~\fade{\&}~\smTMtransR{\blank}{1}} (0);
      \path (1) edge                 node[swap]   {\smTMtransL{1}{0}}                                  (2);
      \path (2) edge[bend right]     node[swap]   {\smAlign{\smTMtransR{0}{0}\smTMtransR{\blank}{0}}}  (1);
